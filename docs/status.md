...
| Typecheck | `uv run mypy` (strict, 56 files) | 0 |
...
| Migrations up | `uv run alembic upgrade head` | 0 |
| Migrations down | `uv run alembic downgrade base` | 0 |
| Offline demo | `uv run tradeup demo --quiet` | 0 |

`make verify` (lint + typecheck + coverage + demo) exits 0.

## Tests

**446 tests pass.** None requires the network. Hypothesis runs in `derandomize` mode.

| Suite | Covers |
|---|---|
| `tests/unit/` | Money, items, rules, mathematics, fees, valuation, EV, policy, inventory, ledger, reservations, metadata, discovery, optimizer |
...
Overall **85.3%** (floor 80%). Money-critical modules, floor 90%:
...
| `valuation/expected_value.py` | 98.0% |
...
| G13 No concealed incompleteness | Pass | `check_no_placeholders.py` over 56 source files |
...
3. **Fee schedules are synthetic.** The only fee data in the repository is the demo's
   ...
   before any live evaluation means anything.
